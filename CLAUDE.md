...
### Tool Location
The llm-context management tool is installed at: ``

To use the tool from this project:
```bash
 add <topic>    # Add a new topic
 scan           # Re-scan project to update topics
 list           # List all topics
 show <topic>   # Show topic content
```
...
When you need to understand the codebase structure, navigate to files, or find relevant code:
1. First read the code structure: `./ctx code-structure`
2. This provides a map of all functions, classes, and methods with line numbers
3. Then use Read on specific files for implementation details

The `code-structure` topic is auto-generated by tree-sitter and updated on every commit via a pre-commit hook.
